...
struct Matrix4 {
    ...
    static Matrix4<T> Perspective(T fovy_deg, T aspect, T near, T far)
    {//OpenGL Ref에 나오는 행렬의 전치 행렬로 작성한다.
        T f = 1.0 / std::tan((double)fovy_deg*0.0087266444);// =1/180.0*3.141592/2.0
        T a = f/aspect;  //[0,0]
        T b = f;         //[1,1]
        T c = (far+near)/(near-far);    //[2,2]
        T d = (2.0 * far * near)/(near-far); //[2,3]
        
        Matrix4 m;
        m.x.x = a; m.x.y = 0; m.x.z = 0; m.x.w = 0;
        m.y.x = 0; m.y.y = b; m.y.z = 0; m.y.w = 0;
        m.z.x = 0; m.z.y = 0; m.z.z = c; m.z.w = -1;
        m.w.x = 0; m.w.y = 0; m.w.z = d; m.w.w = 0;
        return m;
    }    
    static Matrix4<T> Ortho(T left, T right, T bottom, T top, T nearVal, T farVal)
    {//OpenGL Ref에 나오는 행렬의 전치 행렬로 작성한다.
        T tx = (right+left)/(left-right);
        T ty = (top+bottom)/(bottom-top);
        T tz = (farVal+nearVal)/(nearVal-farVal);
        
        Matrix4 m;
        m.x.x = 2.f/(right-left);   m.x.y = 0;                  m.x.z = 0;                      m.x.w = 0;
        m.y.x = 0;                  m.y.y = 2.f/(top-bottom);   m.y.z = 0;                      m.y.w = 0;
        m.z.x = 0;                  m.z.y = 0;                  m.z.z = 2.f/(nearVal-farVal);   m.z.w = 0;
        m.w.x = tx;                 m.w.y = ty;                 m.w.z = tz;                     m.w.w = 1;
        return m;
    }    
    static Matrix4<T> LookAt(vec3 eye, vec3 center, vec3 up)
    {//OpenGL Ref에 나오는 행렬의 전치 행렬로 작성한다.
        vec3    F = center - eye;
        F.Normalize();
        up.Normalize();
        ...
        m.w.x = 0;     m.w.y = 0;     m.w.z = 0;     m.w.w = 1.f;
        return m;
    }    
    vec4 x;
    vec4 y;
