...
@interface ShowAllSBMViewController ()<DropdownMenuDelegate,UIGestureRecognizerDelegate,MADelegate>{
...
@implementation ShowAllSBMViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"显示所有的sbm";
...
- (void)addChangeBackColorMenu{
    ...
    NSArray *array = @[@"所有",@"1.5",@"0.5"];
    [dropdownMenu.mainBtn setTitle:@"SBM" forState:UIControlStateNormal];
    ...
    if (menu == self.changeBackColorMenu) {
        ...
        if (0 == number) {//  所有sbm
             [_manager loadMapAll:[[_manager getDefaultBuildingId]intValue]];
              ...
              [_manager setVerticalScale:1.5];
           
        }else{
              [_manager setVerticalScale:0.3];
