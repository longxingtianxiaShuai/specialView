...
@interface SetBackgColorViewController ()<DropdownMenuDelegate,MADelegate>
...
@end
@implementation SetBackgColorViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"设置视图的背景颜色";
}
...
- (void)addChangeFloorMenu{
    ...
    [dropdownMenu.mainBtn setTitle:@"切换楼层" forState:UIControlStateNormal];
...
- (void)addChangeBackColorMenu{
    ...
    NSArray *array = @[@"灰色",@"橘色",@"青色"];
    [dropdownMenu.mainBtn setTitle:@"切换颜色" forState:UIControlStateNormal];
...
- (void)dropdownMenu:(DropdownMenu *)menu selectedCellNumber:(NSInteger)number{
    ...
    if (menu == self.changeBackColorMenu) {
        NSLog(@"改变颜色");
        if (0 == number) {
            [self.manager setBackgroundColorWithRed:220.0 green:220.0 blue:220.0];
        }else if (1 == number){
            [self.manager setBackgroundColorWithRed:238.0 green:180.0 blue:34.0];
        }else{
            [self.manager setBackgroundColorWithRed:144.0 green:238.0 blue:144.0];
        }
    }else{
         NSLog(@"改变楼层");
