//
//  GuidedSimulationViewController.m
//  SDKUsage
//
...
@interface GuidedSimulationViewController ()<DropdownMenuDelegate,UIGestureRecognizerDelegate,MADelegate,MAGuidanceDelegate>{
    
    CGFloat _origional;
    vbPathNode *node;
    std::vector<vbPathNode> nodes;
}
...
@property (nonatomic,strong)MAPoi *departure;
@property (nonatomic,strong)MAPoi *arrival;
@property (nonatomic,strong)VBPathGuider *pathGuider;
@end

@implementation GuidedSimulationViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"模拟导航";
...
- (void)dropdownMenu:(DropdownMenu *)menu selectedCellNumber:(NSInteger)number{
    ...
    if (menu == self.changeBackColorMenu) {
        ...
        if (0 == number) {//  显示路径
            ...
            NSLog(@"poi0.poi_id=== %zd,poi3.poi_id==== %zd",poi0.poi_id,poi3.poi_id);
...
- (void) findPathDeparture:(MAPoi *)departure toArrival:(MAPoi *)arrival via:(MAPoi *)via rest:(BOOL)rest
{
    ...
    // [_manager showPoiByInfo:pois show:YES];
    [_manager getGmlPath];
...
- (void)guide{
    ...
    nodes.clear();
    nodes.size();
    self.pathGuider = [[VBPathGuider alloc] initWithNaviMode:vbPath_NaviMode_BezierTurn];
   float deg = [self.pathGuider getDirDegreeFrom:node[0] toNode:node[3]];
    NSLog(@"deg===   %f",deg);
    self.pathGuider.delegate = self;
    ...
    [self.pathGuider setSpeed:50 turn:10];
    
    BOOL isSet =  [self.pathGuider setPathNode:&nodes];
    NSLog(@"isSet===   %zd",isSet);
    [self.pathGuider doNextStep];
    [self.pathGuider initGuide];
    [self.pathGuider doNextStepTurnBezier];
    //[self.pathGuider resolvePauseState];
    
  //  [self.pathGuider getCurPos];
    //float degree = self.pathGuider.getCurDirDeg;
   // self.pathGuider.doNextStep;
    //[self.pathGuider setSpeed:degree turn:1];
   // [self.pathGuider updateAheadPosAndOrientation];
   // _manager.setCameraYaw(degree);
    
}
#pragma mark guidanceDelegate
