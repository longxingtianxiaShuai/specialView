...
@interface MyLocationViewController ()<DropdownMenuDelegate,UIGestureRecognizerDelegate,MADelegate>{
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"显示隐藏我的位置";
...
- (void)addChangeBackColorMenu{
    ...
    NSArray *array = @[@"显示",@"隐藏"];
    [dropdownMenu.mainBtn setTitle:@"我的位置" forState:UIControlStateNormal];
...
//            NSLog(@"categoryA====  %@  cate==  %@"  ,categoryA,cate);
            ...
            MAVector *vector = [_manager getMapPointX:100 andY:100];
            [_manager showDynamicPoi:YES x:vector.x y:vector.y z:vector.z];
            //[_manager showMyPos:YES x:vector.x y:vector.y z:vector.z red:0.8 green:0.5 blue:0.9 alpha:1 rad:100];
//            - (void)VBShowMyPos:(BOOL)show x:(float)x y:(float)y z:(float)z red:(float)red green:(float)green blue:(float)blue alpha:(float)alpha rad:(float)rad;
        }else if (1 == number){ // 顶部
           // [_manager showPoi:NO];
