...
@interface CaneraModeViewController ()<DropdownMenuDelegate,UIGestureRecognizerDelegate,MADelegate>{
...
@implementation CaneraModeViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"设置相机模式";
...
- (void)addChangeBackColorMenu{
    ...
    NSArray *array = @[@"鸟瞰",@"顶部",@"围绕"];
    [dropdownMenu.mainBtn setTitle:@"相机模式" forState:UIControlStateNormal];
...
- (void)dropdownMenu:(DropdownMenu *)menu selectedCellNumber:(NSInteger)number{
    ...
    if (menu == self.changeBackColorMenu) {
        NSLog(@"切换角度");
        if (0 == number) {// 鸟瞰
            [_manager setCameraMode:eBirdView];
            
        }else if (1 == number){ // 顶部
            [_manager setCameraMode:eTopView];
        }else{ // 围绕
            [_manager setCameraMode:eWalkingView];
