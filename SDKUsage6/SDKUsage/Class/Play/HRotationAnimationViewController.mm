//
//  HRotationAnimationViewController.m
//  SDKUsage
//
...
@interface HRotationAnimationViewController ()<DropdownMenuDelegate,UIGestureRecognizerDelegate,MADelegate>{
...
@implementation HRotationAnimationViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.navigationItem.title = @"自动旋转";
...
- (void)addChangeBackColorMenu{
    ...
    NSArray *array = @[@"执行",@"停止"];
    [dropdownMenu.mainBtn setTitle:@"动画" forState:UIControlStateNormal];
...
- (void)dropdownMenu:(DropdownMenu *)menu selectedCellNumber:(NSInteger)number{
    ...
    if (menu == self.changeBackColorMenu) {
        ...
        if (0 == number) {  //执行动画
            ...
            [self setHRotationAnimation:YES step:-1.0f];
          
            
          
            
        }else if (1 == number){ // 停止动画
            [self setHRotationAnimation:NO step:-1.0f];
...
- (void) setHRotationAnimation:(BOOL)play step:(float)value
{
    [_manager setHRotationAnimation:play step:value];
}
@end
